...
def host_key():
    ...
    require(
        ...
        "credential_directory_permissions_invalid",
    )
...
def parse_manifest(path, expected_sha):
    ...
    require(
        ...
        and plan.get("workspace_uuid") == WORKSPACE_UUID,
    ...
    require(
        type(target["size_bytes"]) is int and 0 < target["size_bytes"] <= MAX_ARCHIVE,
        "target_size_invalid",
    )
    require(
        type(target["file_count"]) is int and 0 < target["file_count"] <= 10000,
    ...
    require(
        type(plan["payload_bytes"]) is int and 0 < plan["payload_bytes"] <= maximum,
    ...
    for item in chunks:
        ...
        require(
            type(item["size_bytes"]) is int and 0 < item["size_bytes"] <= PART_BYTES,
            "chunk_size_invalid",
        )
    ...
    if plan["transport"] == "github_delta":
        ...
        require(
            plan["payload_sha256"] == target["sha256"]
            and plan["payload_bytes"] == target["size_bytes"],
            "full_payload_target_mismatch",
        )
    return plan
...
def verified_source(rows, target):
    ...
    row = matches[0]
    require(
        row.get("state") == "verified" and row.get("size_bytes") == target["size_bytes"],
...
def assemble(delivery, plan, work):
    payload = b"".join(
        read_bounded(safe_path(delivery / item["path"]), PART_BYTES) for item in plan["chunks"]
    )
    offset = 0
    for item in plan["chunks"]:
        part = payload[offset : offset + item["size_bytes"]]
        require(
            len(part) == item["size_bytes"] and sha(part) == item["sha256"], "chunk_digest_drift"
        )
        offset += len(part)
    require(
        len(payload) == plan["payload_bytes"] and sha(payload) == plan["payload_sha256"],
        "payload_digest_drift",
    )
    output = safe_path(work / "payload.zip")
    if output.exists():
        require(
            sha(read_bounded(output, MAX_ARCHIVE)) == plan["payload_sha256"], "cached_payload_drift"
        )
    else:
    ...
    if plan["transport"] == "github_delta":
        ...
        if not output.exists():
            apply_delta(
                ...
                patch_sha256=plan["payload_sha256"],
                target_sha256=target["sha256"],
            )
    content = read_bounded(output, MAX_ARCHIVE)
    require(
        len(content) == target["size_bytes"] and sha(content) == target["sha256"],
        "target_digest_drift",
    )
...
def run(action, delivery=None, manifest_sha=None):
    ...
    try:
        ...
        if plan["transport"] == "github_delta":
            ...
            require(
                base and base["id"] == plan["base"]["id"] and base["uuid"] == plan["base"]["uuid"],
        ...
        result = {
            "phase": "reconstruction_verified" if action == "verify" else "source_verified",
