...
正式 Source 使用 4 MiB 分片的可恢復上傳，單一壓縮歸檔上限為 3 GiB，且仍受工作區
剩餘邏輯配額與解封後容量門檻約束。舊版單請求 multipart 相容接口維持 100 MiB；大於
100 MiB 的 Source 必須使用官方 `dm.py source push`，平台不得先把整包讀入記憶體。
