...
const Page = ({ civilizationApp = false } = {}) => {
  ...
  const mobileThoughts = visible.map(thought => { const domain = domainOf(thought.domain); return {
  ...
  const openNewThought = () => { setComposerError(""); setComposer({ initial: null }); };
  const editThought = thought => { setComposerError(""); setComposer({ initial: thought }); };
  ...
  return <div className={"civilization-page" + (civilizationApp ? " civilization-app-page" : "")} data-domain={activeDomain.key} style={domainStyle(activeDomain)}>
    {civilizationApp ? (MobileView ? <MobileView view={view} onView={setView} domains={mobileDomains} activeFilter={filter} onFilter={setFilter} query={query} onQuery={setQuery} thoughts={mobileThoughts} allCount={allThoughts.length} lensCount={totalLensCount} selectedId={selected && selected.id} readerModel={mobileReader} loadError={loadError} onRetry={loadThoughts} onNew={openNewThought} onCli={() => W2.openBusinessAction("civilization_api_key_issue")} onRead={id => selectThought(id, "c")} onShare={openShare} onEdit={editThought}/> : <div className="civ-empty">{t("正在讀取文明資料")}…</div>) : <>
