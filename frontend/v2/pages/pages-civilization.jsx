...
window.W2_LANG.addEN({
  ...
  "我的讀書筆記": "My reading notes", "寫一篇讀書筆記": "Write a reading note",
  "創作讀書筆記": "Create a reading note", "編輯讀書筆記": "Edit reading note",
  "筆記標題": "Note title", "一句摘要": "One-line summary", "我的思考": "My reflection", "筆記正文": "Note body",
  "草稿不會公開；發布後公司成員可閱讀，是否公開分享到網頁由你另行決定。": "Drafts are never public. Company members can read a published note, and you decide separately whether to share it on the public web.",
  "讀書筆記已發布": "Reading note published",
...
const CivilizationComposer = ({ busy, error, initial, mode = "thought", onClose, onSave }) => {
  const editing = !!initial;
  const personalNote = mode === "reading-note";
  ...
  const submit = event => {
    ...
    }, initial, intent === "publish", personalNote);
  };
  ...
  return <div className="civ-composer" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget && !busy) onClose(); }}>
    <form className="civ-composer-card" role="dialog" aria-modal="true" aria-labelledby="civ-composer-title" onSubmit={submit}>
      <header className="civ-composer-head"><b>{personalNote ? "N1" : "C1"}</b><div><span className="civ-eyebrow">{personalNote ? (editing ? "REVISE READING NOTE" : "NEW READING NOTE") : (editing ? "REVISE THOUGHT OBJECT" : "NEW THOUGHT OBJECT")}</span><h2 id="civ-composer-title">{personalNote ? t(editing ? "編輯讀書筆記" : "創作讀書筆記") : t(editing ? "編輯思考" : "記錄問題")}</h2></div><button type="button" disabled={busy} className="civ-composer-close" onClick={onClose} aria-label={t("取消")}>×</button></header>
      <div className="civ-composer-body">
        ...
        <label>{t("刊頭文字")}<input name="eyebrow" maxLength="100" defaultValue={editing ? (editorContent.eyebrow || "CIVILIZATION · QUESTION") : (personalNote ? "CIVILIZATION · READING NOTE" : "CIVILIZATION · QUESTION")}/></label>
      ...
      <footer className="civ-composer-foot"><span>{t(personalNote ? "草稿不會公開；發布後公司成員可閱讀，是否公開分享到網頁由你另行決定。" : "保存後公司成員可見；建立者與公司管理員可繼續編輯。")}</span><div><button type="submit" name="intent" value="draft" disabled={busy}>{busy ? t("正在保存") : t("保存草稿")}</button><button type="submit" name="intent" value="publish" className="is-publish" disabled={busy}>{busy ? t("正在發布") : t("發布這一版")}</button></div></footer>
    </form>
  </div>;
};
...
const Page = ({ civilizationApp = false } = {}) => {
  const initialMemory = useMemo(readMemory, []);
  const params = useMemo(routeParams, []);
  const supportedViews = civilizationApp ? ["a", "b", "c", "d"] : ["a", "b", "c"];
  const [view, setView] = useState(supportedViews.includes(params.get("view")) ? params.get("view") : (supportedViews.includes(initialMemory.view) ? initialMemory.view : "a"));
  ...
  const saveThought = async (value, initial, publish, personalNote = false) => {
    ...
    try {
      if (initial) {
        ...
        setSelectedId(finalResult.thought.id); setComposer(null); setToast(t(publish && personalNote ? "讀書筆記已發布" : (publish ? "問題已發布" : "草稿已保存")));
      } else {
        const result = await W2.post("/api/civilization/thoughts", { ...value, publish });
        setThoughts(current => (current || []).concat(result.thought));
        setSelectedId(result.thought.id); setComposer(null); setToast(t(publish && personalNote ? "讀書筆記已發布" : (publish ? "問題已發布" : "草稿已保存")));
      }
  ...
  if (thoughts === null) return <div className={"civilization-page" + (civilizationApp ? " civilization-app-page" : "")}><div className="civ-empty">{t("正在讀取文明資料")}…</div></div>;
  ...
  const atlas = <div className="civ-atlas-layout">
    ...
    {selected && <aside className="civ-atlas-detail" data-domain={selectedDomain.key} style={domainStyle(selectedDomain)}>
      ...
      <div className="civ-detail-actions"><button type="button" onClick={() => setView("c")}>{t("海報閱讀")} →</button><button type="button" onClick={() => openShare(selected)}>{t("分享")} ↗</button>{selected.can_edit && <button type="button" onClick={() => editThought(selected)}>{t("編輯")} ↗</button>}<button type="button" onClick={() => selected.can_delete ? removeThought(selected) : copyLink(selected)}>{selected.can_delete ? t("刪除") : t("複製分享")} ↗</button></div>
    </aside>}
  </div>;
  ...
  const reader = !selected ? <div className="civ-empty">{t("沒有符合條件的思考對象")}</div> : <div className={"civ-reader" + (notesOpen ? "" : " is-notes-collapsed")} data-domain={selectedDomain.key} style={domainStyle(selectedDomain)}>
    ...
    {notesOpen && <aside className="civ-reader-notes" data-domain={selectedDomain.key} style={domainStyle(selectedDomain)}><div className="civ-reader-notes-head"><span className="civ-micro">CHANGE THE LENS / {t("換一個角度")}</span><button type="button" data-testid="civilization-notes-close" aria-label={t("收起閱讀工具欄")} title={t("收起閱讀工具欄")} onClick={() => setNotesOpen(false)}>×</button></div><h3>{selectedContent.title || thoughtText(selected, "title")}</h3><p>{selectedContent.quote || selectedContent.short || thoughtText(selected, "short")}</p><div className="civ-publication-meta"><span><b>{t("固定版面")}</b>SWISS B / V1</span><span><b>{t("發布版本")}</b>{String(selected.published_revision || 0).padStart(2, "0")}{selected.has_draft ? ` · ${t("草稿")}` : ` · ${t("已發布")}`}</span></div><div className="civ-lens-list">{selectedLenses.length ? selectedLenses.map((item, index) => <button type="button" key={index} className={lens === index ? "is-active" : ""} onClick={() => setLens(index)}><b>{String(index + 1).padStart(2, "0")}</b><span><strong>{localText(item.name)}</strong><br/>{localText(item.text)}</span></button>) : <div className="civ-empty civ-lens-empty">{t("尚未添加視角")}</div>}</div><div className="civ-read-actions"><button type="button" className="civ-read-action" onClick={() => openShare(selected)}>{t("分享")} →</button>{selected.can_edit && <button type="button" className="civ-read-action" onClick={() => editThought(selected)}>{t(selectedLenses.length ? "編輯" : "添加視角")} →</button>}<button type="button" className="civ-read-action" onClick={() => selected.can_delete ? removeThought(selected) : copyLink(selected)}>{selected.can_delete ? t("刪除") : t("複製這個思考的鏈接")} →</button></div></aside>}
  </div>;

  const MobileView = W2.CivilizationMobileA;
  const mobileDomains = DOMAINS.map(domain => ({ key: domain.key, label: lang() === "en" ? domain.en : t(domain.zh), style: domainStyle(domain) }));
  const toMobileThought = thought => { const domain = domainOf(thought.domain); return {
    id: thought.id, no: thought.no, date: thought.date, domainKey: domain.key,
    domainLabel: lang() === "en" ? domain.en : t(domain.zh), domainEn: domain.en,
    title: thoughtText(thought, "title"), short: thoughtText(thought, "short"),
    lensCount: thoughtLenses(thought).length, relations: thoughtRelations(thought).map(localText), canEdit: !!thought.can_edit,
    isMine: !!thought.is_mine, publicationStatus: thought.publication_status, isPublic: !!thought.public_share_enabled,
    style: domainStyle(domain), source: thought,
  }; };
  const mobileThoughts = visible.map(toMobileThought);
  const mobileOwnNotes = allThoughts.filter(thought => thought.is_mine).map(toMobileThought).reverse();
  const mobileReader = selected ? {
    id: selected.id, no: selected.no, date: selected.date, domainKey: selectedDomain.key,
    domainLabel: lang() === "en" ? selectedDomain.en : t(selectedDomain.zh), domainEn: selectedDomain.en,
    title: selectedContent.title || thoughtText(selected, "title"), short: selectedContent.short || thoughtText(selected, "short"),
    quote: selectedQuote, thesis: selectedContent.thesis || thoughtText(selected, "thesis"),
    sections: selectedReadingSections.map((section, index) => ({
      marker: section.marker || String(index + 1).padStart(2, "0"), kicker: section.kicker || "SECTION / THOUGHT", heading: section.heading,
      paragraphs: Array.isArray(section.paragraphs) ? section.paragraphs : proseParagraphs(section.paragraphs),
    })),
    lenses: selectedLenses.map(item => ({ name: localText(item.name), text: localText(item.text) })),
    canEdit: !!selected.can_edit, style: domainStyle(selectedDomain), source: selected,
  } : null;
  const totalLensCount = allThoughts.reduce((sum, item) => sum + thoughtLenses(item).length, 0);
  const openNewReadingNote = () => { setComposerError(""); setComposer({ initial: null, mode: "reading-note" }); };
  const editThought = thought => { setComposerError(""); setComposer({ initial: thought, mode: thought.is_mine ? "reading-note" : "thought" }); };

  return <div className={"civilization-page" + (civilizationApp ? " civilization-app-page" : "")} data-domain={activeDomain.key} style={domainStyle(activeDomain)}>
    {civilizationApp ? (MobileView ? <MobileView view={view} onView={setView} domains={mobileDomains} activeFilter={filter} onFilter={setFilter} query={query} onQuery={setQuery} thoughts={mobileThoughts} myNotes={mobileOwnNotes} allCount={allThoughts.length} lensCount={totalLensCount} selectedId={selected && selected.id} readerModel={mobileReader} loadError={loadError} onRetry={loadThoughts} onNew={openNewReadingNote} onCli={() => W2.openBusinessAction("civilization_api_key_issue")} onRead={id => selectThought(id, "c")} onShare={openShare} onEdit={editThought}/> : <div className="civ-empty">{t("正在讀取文明資料")}…</div>) : <>
    <header className="civ-toolbar"><div className="civ-toolbar-brand"><span className="civ-toolbar-mark" aria-hidden="true"/><span>BONFIRE PLATFORM<br/>CIVILIZATION / {t("文明")}</span></div><nav className="civ-view-switch" aria-label={t("閱讀海報")}>{[["a", "問題拓撲"], ["b", "思想時間軸"], ["c", "閱讀海報"]].map(([id, label]) => <button type="button" key={id} aria-selected={view === id} onClick={() => setView(id)}>{id.toUpperCase()} {t(label)}</button>)}</nav><div className="civ-toolbar-right"><span>{String(allThoughts.length).padStart(2, "0")} QUESTIONS · {String(allThoughts.reduce((sum, item) => sum + thoughtLenses(item).length, 0)).padStart(2, "0")} LENSES</span><button type="button" className="civ-cli-button" onClick={() => W2.openBusinessAction("civilization_api_key_issue")}>{t("文明 CLI / API")}</button><button type="button" className="civ-new-button" onClick={openNewReadingNote}>＋ {t("我的讀書筆記")}</button></div></header>
    <section className="civ-masthead"><div className="civ-mast-copy"><span className="civ-kicker">MODULE C1 · {t("共享觀看方式").toUpperCase()}</span><h1 className="civ-mast-title">{t("文明")}<span className="civ-mast-word" aria-label="CIVILIZATION"><i className="civ-mast-word-bars" aria-hidden="true"><b/><b/><b/></i><span className="civ-mast-word-layer civ-mast-word-offset is-red" aria-hidden="true">CIVILIZATION</span><span className="civ-mast-word-layer civ-mast-word-offset is-blue" aria-hidden="true">CIVILIZATION</span><span className="civ-mast-word-layer civ-mast-word-offset is-green" aria-hidden="true">CIVILIZATION</span><span className="civ-mast-word-layer civ-mast-word-color" aria-hidden="true">CIVILIZATION</span><span className="civ-mast-word-layer civ-mast-word-outline" aria-hidden="true">CIVILIZATION</span></span></h1><p className="civ-mast-lead">{t("分享答案之前，先分享我們如何提出問題。")} {t("這裡保存觀察世界的方法、判斷事物的尺度，以及思想彼此連接和變化的過程。")}</p></div><aside className="civ-mast-index"><span className="civ-micro">LIVE INDEX / {new Date().getFullYear()}</span><strong>C1</strong><div className="civ-domain-spectrum" aria-hidden="true">{DOMAINS.slice(1).map(domain => <i key={domain.key} style={domainStyle(domain)}/>)}</div><p>{t("提問").toUpperCase()}<br/>{t("視角").toUpperCase()}<br/>{t("方法").toUpperCase()}<br/>{t("譜系").toUpperCase()}</p></aside></section>
