...
const Page = () => {
  ...
  const reader = !selected ? <div className="civ-empty">{t("沒有符合條件的思考對象")}</div> : <div className="civ-reader" data-domain={selectedDomain.key} style={domainStyle(selectedDomain)}>
    ...
    <article className="civ-reader-poster" data-domain={selectedDomain.key} style={domainStyle(selectedDomain)}><PosterMotion domain={selectedDomain}/><div className="civ-poster-label"><span>CIVILIZATION · QUESTION {selected.no}</span><span>{selectedDomain.en.toUpperCase()}</span></div><h2 className="civ-poster-question"><PosterQuestion>{thoughtText(selected, "title")}</PosterQuestion></h2><p className="civ-poster-thesis">{thoughtText(selected, "thesis")}</p><aside className="civ-poster-index"><span className="civ-micro">LENS INDEX / {String(selectedLenses.length).padStart(2, "0")}</span>{selectedLenses.length ? selectedLenses.slice(0, 3).map((item, index) => <span key={index}><b>{String(index + 1).padStart(2, "0")}</b>{localText(item.name)}</span>) : <span><b>00</b>{t("尚未添加視角")}</span>}</aside><footer className="civ-poster-foot"><span>12 COLUMN SYSTEM · ONE QUESTION / MANY LENSES</span><span>{selected.date}</span></footer></article>
