...
const Page = () => {
  ...
  return <div className="civilization-page" data-domain={activeDomain.key} style={domainStyle(activeDomain)}>
    <div className="civ-mobile-only">{MobileView && <MobileView view={view} onView={setView} domains={mobileDomains} activeFilter={filter} onFilter={setFilter} query={query} onQuery={setQuery} thoughts={mobileThoughts} allCount={allThoughts.length} lensCount={totalLensCount} chronology={chronology} reader={reader} loadError={loadError} onRetry={loadThoughts} onNew={openNewThought} onCli={() => W2.openBusinessAction("civilization_api_key_issue")} onRead={id => selectThought(id, "c")} onShare={openShare} onEdit={editThought}/>}</div>
    <div className="civ-desktop-only">
