...
MK7_PUBLIC_TENANT_ID = UUID("00000000-0000-4000-8000-000000000001")
WORKSPACE_JOB_SPECS = {
    "alembic-upgrade": {
...
def deploy_workspace_latest(asset_no: str) -> dict[str, object]:
    ...
    return configure_workspace_runtime(
        credential,
        {
            ...
            "deploy": True,
            "idempotency_key": (
                f"operator-stage-source-{artifact['version_id']}-runtime-env-v4"
            ),
        },
        get_settings(),
    )
...
def accept_workspace_deployment(
    ...
    """Probe one healthy candidate over the private Runtime network before routing."""

    tenant_id, target = _target(asset_no)
    try:
        resolved_id = UUID(deployment_id)
    except ValueError as exc:
        raise ValueError("Invalid deployment id") from exc
    match = re.fullmatch(r"([0-9]{1,4}):([0-9]{1,6})", expectation)
    if match is None:
        raise ValueError("Expected counts must use MODULES:RESOURCES")
    expected_modules, expected_resources = (int(value) for value in match.groups())
    with tenant_session(tenant_id) as session:
        candidate = (
            session.execute(
                text(
                    """
                    SELECT d.id,d.source_version_id,d.status,d.health,d.result,
                           (w.active_deployment_id=d.id) AS active
                    FROM digital_asset.deployments AS d
                    JOIN digital_asset.workspaces AS w ON w.id=d.workspace_id
                    WHERE d.id=:deployment_id AND d.workspace_id=:workspace_id
                    """
                ),
                {
        ...
        if binding is None:
            raise RuntimeError("Candidate workspace database is unavailable")
        with hosted_database.binding_connection(
            session,
            dict(binding),
            get_settings(),
        ) as database_connection:
            # MK7 forces RLS even for its database owner. Mirror the anonymous
            # public API's server-established tenant context before comparing
            # catalog facts; without it PostgreSQL correctly exposes zero rows.
            database_connection.execute(
                "SELECT set_config('app.tenant_id', %s, true)",
                (str(MK7_PUBLIC_TENANT_ID),),
            )
            database_counts = database_connection.execute(
                """
                SELECT
                  (SELECT count(*) FROM catalog.knowledge_modules)::integer AS modules,
                  (SELECT count(*) FROM catalog.learning_resources)::integer AS resources,
                  (SELECT count(*) FROM catalog.resource_modules)::integer AS directory_links
                """
            ).fetchone()
    if candidate is None:
        raise ValueError("Deployment does not belong to the target workspace")
    if candidate["status"] != "ready" or candidate["health"] != "healthy":
        raise RuntimeError("Only a healthy candidate can be accepted")
    ...
    if str(result.get("execution_mode") or "service") != "service":
        raise RuntimeError("A one-shot job cannot be accepted for traffic")
    internal_url = str(result.get("internal_url") or "").rstrip("/")
    if not re.fullmatch(r"http://[a-zA-Z0-9_.-]+:[0-9]{1,5}", internal_url):
        raise RuntimeError("Candidate internal URL is unavailable")

    module_slugs: list[str] = []
    resource_counts: dict[str, int] = {}
    with httpx.Client(timeout=15.0, follow_redirects=False) as client:
        readiness = client.get(f"{internal_url}/api/v1/health")
        readiness.raise_for_status()
        modules_response = client.get(f"{internal_url}/api/v1/modules")
        modules_response.raise_for_status()
        modules = modules_response.json()
        if not isinstance(modules, list):
            raise RuntimeError("Candidate modules response is invalid")
        for module in modules:
            if not isinstance(module, dict) or not isinstance(module.get("slug"), str):
                raise RuntimeError("Candidate module entry is invalid")
            slug = str(module["slug"])
            module_slugs.append(slug)
            directory_response = client.get(
                f"{internal_url}/api/v1/modules/{quote(slug, safe='-._~')}/resources"
            )
            directory_response.raise_for_status()
            directory = directory_response.json()
            resources = directory.get("resources") if isinstance(directory, dict) else None
            if not isinstance(resources, list):
                raise RuntimeError("Candidate module directory response is invalid")
            resource_counts[slug] = len(resources)

    public_modules = len(modules)
    public_resource_links = sum(resource_counts.values())
    database_modules = int(database_counts["modules"])
    database_resources = int(database_counts["resources"])
    database_directory_links = int(database_counts["directory_links"])
    accepted = (
        public_modules == expected_modules
        and database_modules == expected_modules
        and database_resources == expected_resources
        and database_directory_links == expected_resources
    )
    if not accepted:
        raise RuntimeError(
            "Candidate catalog acceptance failed: "
            f"expected {expected_modules}/{expected_resources}, "
            f"observed public_modules={public_modules}, "
            f"database_modules={database_modules}, "
            f"database_resources={database_resources}, "
            f"database_directory_links={database_directory_links}"
        )
    return _json_safe(
        {
            "ok": True,
            ...
            "observed": {
                ...
                "public_resource_links": public_resource_links,
                "database_modules": database_modules,
                "database_resources": database_resources,
                "database_directory_links": database_directory_links,
                "database_tenant_id": str(MK7_PUBLIC_TENANT_ID),
                "module_slugs": module_slugs,
                "public_resources_by_module": resource_counts,
            },
        }
    )


def activate_workspace(asset_no: str, deployment_id: str) -> dict[str, object]:
    tenant_id, target = _target(asset_no)
    try:
        resolved_id = UUID(deployment_id)
    except ValueError as exc:
        raise ValueError("Invalid deployment id") from exc
    credential = _operator_credential(tenant_id, target["workspace_id"])
    return activate_workspace_deployment(credential, resolved_id)
...
def configure_workspace_environment(
    ...
    """Apply one audited, non-secret Runtime variable through the workspace fabric."""

    tenant_id, target = _target(asset_no)
    credential = _operator_credential(tenant_id, target["workspace_id"])
    normalized_name = name.strip().upper()
    if not re.fullmatch(r"[A-Z][A-Z0-9_]{0,79}", normalized_name):
        raise ValueError("Invalid Runtime environment variable name")
    return apply_fabric_resource(
        credential,
        {
            "kind": "environment",
            "resource_key": "operator-runtime",
            "spec": {"component": "*", "variables": {normalized_name: value}},
        },
        get_settings(),
        idempotency_key=(
            "operator-environment-"
            + hashlib.sha256(f"{normalized_name}\0{value}".encode()).hexdigest()
        ),
    )
...
def run_workspace_job(
    ...
    return configure_workspace_runtime(
        credential,
        {
            ...
            "deploy": True,
            "idempotency_key": (
                f"operator-job-{source_id}-{job_kind}-runtime-role-v3"
            ),
        },
        get_settings(),
    )
...
def main() -> int:
    ...
    elif args.command == "workspace-accept":
        if not args.source_path or not args.version:
            parser.error("workspace-accept requires deployment_id and expected counts")
        result = accept_workspace_deployment(args.target, args.source_path, args.version)
    elif args.command == "workspace-activate":
        if not args.source_path:
            parser.error("workspace-activate requires deployment_id")
        result = activate_workspace(args.target, args.source_path)
    elif args.command == "workspace-environment":
        if not args.source_path or args.version is None:
            parser.error("workspace-environment requires name and value")
        result = configure_workspace_environment(
            args.target,
            args.source_path,
            args.version,
        )
